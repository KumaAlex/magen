<template>
  <div class="flex flex-col justify-between space-y-4">
    <div class="space-y-2 text-sm">
      <div class="flex items-center gap-3">
        <Icon name="mdi:map-marker" class="text-2xl text-gray-200" />
        <span>050059, Республика Казахстан,<br />г. Алматы, ул. Ходжанова 79, 2 этаж</span>
      </div>
      <div class="flex items-center gap-3">
        <Icon name="mdi:phone" class="text-2xl text-gray-200" />
        <span>Офис: +7 (727) 313-13-66</span>
      </div>
      <div class="flex items-center gap-3">
        <Icon name="mdi:cellphone-iphone" class="text-2xl text-gray-200" />
        <span>Моб.: +7 (705) 895-78-38</span>
      </div>
      <div class="flex items-center gap-3">
        <Icon name="mdi:email-edit-outline" class="text-2xl text-gray-200" />
        <span>info@magen.kz</span>
      </div>
      <div class="flex items-center gap-3">
        <Icon name="mdi:web" class="text-2xl text-gray-200" />
        <span>www.magen.kz</span>
      </div>
    </div>

    <div class="pt-6 border-t border-gray-700 text-xs text-gray-400">
      <p class="italic mb-1">Оправдываем доверие!</p>
      <p>&copy; 2025 ТОО Охранное агентство Маген. Все права защищены.</p>
    </div>
  </div>
</template>

<script setup lang="ts">

</script>

<style scoped>

</style>